...
class ResNetMulti(nn.Module):
    def __init__(self, block, layers, num_classes, multi_level):
        ...
        for i in self.bn1.parameters():
            i.requires_grad = False
        self.relu = nn.ReLU(inplace=True)
        self.max_pool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1, ceil_mode=True)  # change
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilation=1):
        downsample = None
        if (stride != 1
                or self.in_planes != planes * block.expansion
                or dilation == 2
                or dilation == 4):
        ...
        layers = []
        layers.append(
            block(self.in_planes, planes, stride, dilation=dilation, downsample=downsample))
